...
export default function Checks({ onBack }: { onBack: () => void }) {
  ...
  if (!data) return <Mono className="text-micro text-paper-low">running the checks…</Mono>
  ...
  return (
    <div className="pb-16">
      <p className="mb-5 flex flex-wrap items-center gap-2">
        ...
        <span className="font-serif text-caption text-paper-mid">A fixture, on purpose.</span>
      </p>
          ...
          {data.faults.map((fault, i) => (
            <li key={fault.label} className="border-t border-rule pt-4">
              <p className="flex items-baseline gap-2">
                <Mono className="text-micro text-paper-low">{String(i + 1).padStart(2, '0')}</Mono>
                <Mono className="text-micro tracking-wide text-falsify/90">rejected</Mono>
              </p>
              <p className="mt-1.5 font-serif text-lede text-paper">{fault.label}</p>
      ...
      <section className="mt-9">
        ...
        <ul className="mt-3">
          {data.survived.unknowns.map((u, i) => (
            <li key={i} className="border-l-2 border-rule pl-3 font-serif text-body text-inferred">
              {u}
            </li>
          ))}
        </ul>
