...
type ChecksResponse = {
  ok: true
  disclaimer: string
  evidence: Array<{ id: string; publisher: string; title: string; url: string; text: string }>
  declaredHoldings: string[]
  faults: Fault[]
  submitted: { headline: string; chain: unknown[]; exposures: unknown[]; quotes: unknown[] }
  ...
  if (failed) {
    return (
      <p className="font-serif text-[15px] leading-relaxed text-inferred">
        The demonstration could not be loaded.
      </p>
    )
  }

  if (!data) return <Mono className="text-[11px] text-paper/40">running the checks…</Mono>

  const { validation: v } = data
  const removed = v.chainDropped + v.exposuresDropped + v.quotesDropped

  return (
    <div className="pb-16">
      <p className="mb-5 flex flex-wrap items-center gap-2">
        <SampleStamp label="Not a Brief" />
        <span className="font-serif text-[13px] text-paper/55">A fixture, on purpose.</span>
      </p>
      ...
      <p className="mt-4 font-serif text-[15px] leading-relaxed text-paper/75">
      ...
      <div className="mt-7 border-y border-rule py-4">
        <p className="font-serif text-[19px] leading-snug text-paper">
          ...
          <span className="text-signal">{v.figuresRedacted}</span> figure struck from the prose.
        </p>
        <dl className="mt-3 grid grid-cols-[9.5rem_1fr] gap-x-4 gap-y-1.5">
          {[
            ['chain links', `${v.chainKept} kept · ${v.chainDropped} removed`],
            ['exposures', `${v.exposuresKept} kept · ${v.exposuresDropped} removed`],
            ['quotes', `${v.quotesKept} kept · ${v.quotesDropped} removed`],
          ].map(([k, val]) => (
            <div key={k} className="contents">
              <dt className="font-mono text-[11px] text-paper/40">{k}</dt>
              <dd className="font-mono text-[11px] text-paper/75">{val}</dd>
            </div>
          ))}
        </dl>
      </div>
      ...
      <section className="mt-9">
        ...
        {data.evidence.map((e) => (
          <div key={e.id}>
            <p className="flex flex-wrap items-baseline gap-x-2">
              ...
              <Mono className="text-[11px] text-paper/60">{e.publisher}</Mono>
            </p>
            <p className="mt-1.5 font-serif text-[15px] leading-snug text-paper/85">{e.title}</p>
            <p className="mt-2 border-l-2 border-signal/40 pl-3 font-serif text-[14px] leading-relaxed text-paper/70">
              {e.text}
            </p>
          </div>
        ))}
        <p className="mt-3 font-serif text-[13px] leading-relaxed text-paper/50">
          ...
          {data.declaredHoldings.map((h) => (
            <Mono key={h} className="text-[12px] text-paper/70">
              {h}{' '}
            </Mono>
          ))}
        </p>
      ...
      <section className="mt-9">
        <h2 className="mb-1 font-serif text-[13px] font-semibold tracking-wide text-paper/50">
          ...
          {data.faults.map((fault, i) => (
            <li key={fault.label} className="border-t border-rule pt-4">
              <p className="flex items-baseline gap-2">
                ...
                <Mono className="text-[10px] tracking-wide text-falsify/90">rejected</Mono>
              </p>
              <p className="mt-1.5 font-serif text-[16px] leading-snug text-paper">{fault.label}</p>
              <p className="mt-1.5 font-serif text-[15px] leading-relaxed text-paper/70">
                {fault.detail}
              </p>
            </li>
          ))}
      ...
      <section className="mt-9">
        ...
        <p className="mt-3 font-serif text-[14px] leading-relaxed text-paper/60">
          One chain link — an inference, honestly labelled as one, citing nothing
          and claiming no figures. One exposure, for a holding the reader actually
          declared. One quote, word for word in the article. And where the model
          offered no gaps, the absence is reported rather than hidden:
        </p>
        <ul className="mt-3">
          {data.survived.unknowns.map((u, i) => (
            <li key={i} className="border-l-2 border-rule pl-3 font-serif text-[15px] leading-relaxed text-inferred">
              {u}
            </li>
          ))}
        </ul>
      ...
      <button
        type="button"
        onClick={onBack}
        className="mt-9 w-full rounded-[2px] border border-rule py-3 font-serif text-[15px] text-paper/70 hover:border-paper/30"
      >
        Back
      </button>
    </div>
